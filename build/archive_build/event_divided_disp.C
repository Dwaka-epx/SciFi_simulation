...
void DrawTrueStepping(TCanvas *cstep, TTree *tree2, int i);

void event_divided_disp(TString fname="mydata")
{
...
//	true trajectry tepping point
   ...
   TCanvas *cdisp = new TCanvas("cdisp","cdisp",500,600);
   TCanvas *cstep = new TCanvas("cstep","cstep",500,600);
   cdisp->Divide(1,2);
	cdisp->cd(1); grYZ->SetTitle(";z axis [mm]; y axis [mm]");
	cdisp->cd(2); grXZ->SetTitle(";z axis [mm]; x axis [mm]");
	//cdisp->Print(canName+"[", "pdf");

   cstep->Divide(1,2);
	cstep->cd(1); grYZ->SetTitle(";z axis [mm]; y axis [mm]");
	cstep->cd(2); grXZ->SetTitle(";z axis [mm]; x axis [mm]");
	cstep->Print(canName+"[", "pdf");
   ...
   // 
     int ndisplay =0;
     for (int i=0; i<20; i++) {
       if(ndisplay>=15) break; // you can see the last display in c1.
	...
	for (int l = 0; l < actuallayers; l++){ 
	  for (int ID= 0; ID< nfibers_dummy; ID++){ 
   	...
   	cdisp->cd(1); grYZ->Draw("ap"); palette->Draw(); // ok @21/09/14
   	cstep->cd(1); grYZ->Draw("ap"); palette->Draw(); // ok @21/09/14
   	cdisp->cd(2); grXZ->Draw("ap"); palette->Draw(); // ok @21/09/14
   	cstep->cd(2); grXZ->Draw("ap"); palette->Draw(); // ok @21/09/14 
		//return;
      ...
      DrawTrueStepping(cstep, tree2, i);
...
void DrawTrueStepping(TCanvas *cstep, TTree *tree2, int i)
{
      ...
      tree2->SetMarkerColor(NEUTRONCOL);     tree2->Draw(termA.str().data(), termN.str().data(),"same");
      ...
      tree2->SetMarkerColor(NEUTRONCOL);     tree2->Draw(termB.str().data(), termN.str().data(),"same");
