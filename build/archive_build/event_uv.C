/*
cutting step smaller than 1 p.e.
...
void event_uv(TString fname="mydata")
{
  // gStyle->SetPadRightMargin(0.12);
   ...
   grUZ->GetXaxis()->SetLimits(-20, fiber_thickness * actuallayers + 20); // ok @21/09/14
   grVZ->GetXaxis()->SetLimits(-20, fiber_thickness * actuallayers + 20); // ok @21/09/14
   ...
   c1->Divide(2,2);
        ...
        c1->cd(2); grYZ->SetTitle(";z axis [mm]; y axis [mm]");
       	c1->cd(3); grUZ->SetTitle(";z axis [mm]; u axis [mm]");
   ...
   for (int l = 0; l < actuallayers; l++) {
     ...
     for (int i = 0; i < arrXZv3.size(); i++) {
       int z_mother_position = fiber_thickness * l; 
	 ...
	 grXZ->SetPoint(grXZ->GetN(), arrXZv3[i].Z() + z_mother_position, 
       ...
       if ( l%unit==10 ) {
	 int sign = arrUZv3[i].Phi()/abs(arrUZv3[i].Phi());
	 grUZ->SetPoint(grUZ->GetN(), arrUZv3[i].Z() + z_mother_position, 
			arrUZv3[i].Mag()*sign );
       // cout << arrUZv3[i].Z() + z_mother_position << "," << arrUZv3[i].Mag()/* bad guy cuz magnitude*/ << endl;
       }
       if ( l%unit==20 ) {
	 grYZ->SetPoint(grYZ->GetN(), arrYZv3[i].Z() + z_mother_position, 
       ...
       if ( l%unit==30 ) {
	 int sign = arrUZv3[i].Phi()/abs(arrUZv3[i].Phi());
	 grVZ->SetPoint(grVZ->GetN(), arrVZv3[i].Z() + z_mother_position, 
			arrVZv3[i].Mag()*sign);
       }
   ...
   // 
	...
	c1->cd(3); grUZ->Draw("ap"); palette->Draw(); // ok @21/09/14
	...
	for (int l = 0; l < actuallayers; l++) {
	  ...
	  for (int ID= 0; ID< nfibers_dummy; ID++) { 
	    ...
	    cout <<"ID="<< ID << " hit point is ("<< arrXZv3[ID].Z()+z_mother_position<<"," <<  arrXZv3[ID].X() << ")" << endl;
            if( l%unit==0 ) { //
