...
void v3_mac(TString fname= "mydata_neut_5.4.0_675MeV_H2O_numu_CCQE_1e7event"//Form("evntdisplay_pcode%dexample_pitch10mm",myparticleID)//"mydata_neut_5.4.0_675MeV_H2O_numu_1e5event"
	...
	// 0.3 is MIP position
	float maxdEdx1  = 0.50;
	const double henkan = 40.0/0.138/std::sqrt(2); //E to p.e.
	...
	colpale->GetZaxis()->SetTitleSize(title_size);
	colpale->GetZaxis()->SetTitleOffset(0.4);//0.55
	colpale->GetZaxis()->SetLabelSize(label_size);//0.030
	...
	grXZ->GetXaxis()->SetLimits(-20, fiber_thickness *  length_along_Z + 20); // ok @21/09/14
	grYZ->GetXaxis()->SetLimits(-20, fiber_thickness *  length_along_Z + 20); // ok @21/09/14
	grUZ->GetXaxis()->SetLimits(-20, fiber_thickness *  length_along_Z + 20); // ok @21/09/14
	grVZ->GetXaxis()->SetLimits(-20, fiber_thickness *  length_along_Z + 20); // ok @21/09/14
	...
	int ndisplay =0;
	double pe_max = maxphoton*3;
	TH2D *h_2D_dEdz_photon = new TH2D("h_2D_dEdz_photon"
																,"h_2D_dEdz_photon"
																, number_of_actuallayers          //nbinsx
																, offset                          //xlow
																, number_of_actuallayers+offset   //xup
																,pe_max                           //nbinsy
																,1                                //ylow
																,pe_max + offset);                //yup
	h_2D_dEdz_photon->SetTitle(";number of layers; Photon [p.e.]");
	...
	for (int i=0; i<100000; i++) {
		...
		// TH1D *hist_fitX = new TH1D("hist_fitX","hist_fitX"+fname,length_along_Z,offset,length_along_Z+offset);
	...
	if( l%unit==0 ) { //X layer
		double Z_signal = arrXZv3[ID].Z() + z_mother_position;
		double X_signal = arrXZv3[ID].X();
		
		TMarker* mk = new TMarker(Z_signal, X_signal, 21);
		...
		if (photon >= min_photon){
			mk->Draw();
			//	    hist_fitX->Fill(Z_signal, X_signal);
			//cout << "(Z_signal,X_signal)= ("<< Z_signal <<"," << X_signal <<")"<< endl;
		}
	...
	if( l%unit==3 ) { // VZ layer
		...
		// "(Z_signal,V_signal) = ("<< arrVZv3[ID].Z() + z_mother_position << ","<< arrVZv3[ID].Mag()*signV << ")" << endl;
		...
		if (photon >= min_photon){
			...
			h_2D_dEdz_photon->Fill(l-first_hit_layer
												,sum_photon_lth_layer);
...
# if yprojection
	// Drawing Y projection
	int first = h_2D_dEdz_photon->GetXaxis()->FindBin(1.);
	int end  = h_2D_dEdz_photon->GetXaxis()->FindBin(4.);
	TH1D *h_projection = h_2D_dEdz_photon->ProjectionY("Yprojection", first, end);
	h_projection->SetTitle(";Photon [p.e.]; Counts");   
	TCanvas *c_projection = new TCanvas("c_projection","c_projection",1000,600);
	h_projection->Draw();
	c_projection->Print(canName, "pdf");
	TFile *fout = new TFile("yprojection.root","update");
	TObject *hist = static_cast<TH1D*>(tf->Get("Yprojection"))->Clone("Yprojection_"+fname);
	hist->Write();
#endif
