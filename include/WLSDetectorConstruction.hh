...
class WLSDetectorConstruction : public G4VUserDetectorConstruction
{
  public:
    ...
    std::vector<int>      GetReadoutYID() { return fReadoutYID; };
    std::vector<TVector3> GetReadoutY()   { return fReadoutY;   };
    std::vector<TVector3> GetReadoutX()   { return fReadoutX;   };
    std::vector<int>      GetReadoutFID() { return fReadoutFID; };
    std::vector<TVector3> GetReadoutF()   { return fReadoutF;   };

	void DefineLayerStraightFiber(G4LogicalVolume* logi, double fiber_length, double fiber_thickness, double coat_thickness);
	void DefineLayerSparceFiber  (G4LogicalVolume* logi, double fiber_length, double fiber_thickness, double coat_thickness);
	void DefineLayerWavingFiber  (G4LogicalVolume* logi, double fiber_length, double fiber_thickness, double coat_thickness, double gap_thickness, int ite_inside_unit);
  	private:

  	WLSMaterials* fMaterials;

   G4LogicalVolume* fLogiWorld;
	G4LogicalVolume* fLogiScCoat;
   G4LogicalVolume   *flogiScCube;
   G4VPhysicalVolume *fPhysScCube;
   G4VPhysicalVolume* fPhysWorld;
    ...
    G4Cache<WLSPhotonDetSD*> fmppcSD;
	 std::vector<TVector3> fReadoutX;
	 std::vector<int>     fReadoutYID;
	 std::vector<TVector3> fReadoutY;
	 std::vector<int>     fReadoutFID;
	 std::vector<TVector3> fReadoutF;
};
#endif
